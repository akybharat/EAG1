...
async def main(creds_file_path: str,
               token_path: str):
    creds_file_path = rf"D:\workspace\code\EAG1\gmail_cred.json"
    token_path = rf"D:\workspace\code\EAG1\application_token\app_tokens.json"
...
if __name__ == "__main__":
    # Get credentials from project directory or environment variables
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.abspath(os.path.join(current_dir, '..', '..', '..',))
    
    # Check for credentials in various locations
    creds_file = os.path.join(project_root, 'gmail_cred.json')
    token_file = os.path.join(project_root, 'token.json')

    creds_file = rf"D:\workspace\code\EAG1\gmail_cred.json"
    token_file = rf"D:\workspace\code\EAG1\application_token\app_tokens.json"
    
    # Also check for environment variables
    creds_file_env = os.getenv('GMAIL_CREDS_FILE')
    token_file_env = os.getenv('GMAIL_TOKEN_FILE')
    
    if creds_file_env and os.path.exists(creds_file_env):
        creds_file = creds_file_env
    if token_file_env and os.path.exists(token_file_env):
        token_file = token_file_env

    # If credentials are still not found, print helpful error message
    if not os.path.exists(creds_file):
        print(f"ERROR: Gmail credentials file not found at {creds_file}")
        print("Please obtain OAuth 2.0 Client ID credentials from Google Cloud Console:")
        ...
        print("4. Download the credentials JSON file and save it as 'gmail_cred.json' in the project root")
        print("   or set the GMAIL_CREDS_FILE environment variable to point to your credentials file")
        sys.exit(1)
    ...
    # Run the main function with the credentials
    asyncio.run(main(creds_file, token_file))

# async def send_test_email():
#     # Initialize the Gmail service with your credentials
#     gmail_service = GmailService(
#         rf"D:\workspace\code\EAG1\gmail_cred.json", 
#         rf"D:\workspace\code\EAG1\application_token\app_tokens.json"
#     )
    
#     # Send email parameters
#     recipient_email = "akybharat02@gmail.com"
#     subject = "Test Email from Python Script"
#     message = "Hello! This is a test email sent from my Python script using the Gmail API."
    
#     # Send the email
#     result = await gmail_service.send_email(recipient_email, subject, message)
#     print(f"Email sending result: {result}")

# # Replace the main call at the bottom of your file with this
# if __name__ == "__main__":
